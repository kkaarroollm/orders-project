...
class MenuItemRepository:
    ...
    async def get_menu_item(self, item_id: str, session: AsyncIOMotorClientSession | None = None) -> MenuItemSchema | None:
    ...
    async def create_menu_item(self, item_data: MenuItemSchema, session: AsyncIOMotorClientSession | None = None) -> str:
        doc = item_data.model_dump(by_alias=True, exclude={"id"})
        result = await self._collection.insert_one(doc, session=session)
        return str(result.inserted_id)

    async def decrement_stock(self, item_id: str, quantity: int, session: AsyncIOMotorClientSession | None = None) -> bool:
    ...
    async def increment_stock(self, item_id: str, quantity: int, session: AsyncIOMotorClientSession | None = None) -> bool:
